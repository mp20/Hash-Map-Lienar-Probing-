...
public class LinearProbingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        if (key == null || value == null) {
            throw new IllegalArgumentException("key/value is(are) null");
        }
        ...
        int counter = 0;
        while (true) {
            if (table[index] != null && table[index].isRemoved()) { //save removed index
                if (counter == 0) {
                    removedIndex = index;
                    counter++;
                }
            }
            if (table[index] == null) { //the spot is empty, so put it in
                if (removedIndex != -1) {
                    table[removedIndex] = new LinearProbingMapEntry<K, V>(key, value);
                } else {
                    table[index] = new LinearProbingMapEntry<K, V>(key, value);
                }
                size++;
                return null;
            } else if (key.equals(table[index].getKey())) { // just replace the value if keys match
    ...
    public V remove(K key) {
        ...
        int index = Math.abs(key.hashCode() % table.length);
        while (true) {
            if (table[index] == null) { // nothing at that location
     ...
     */
    public void clear() {
        LinearProbingMapEntry<K, V>[] rehash = new LinearProbingMapEntry[INITIAL_CAPACITY];
        size = 0;
        table = rehash;
